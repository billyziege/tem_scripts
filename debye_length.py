...
charge_of_macroparticle = args.number_of_electrons_per_macroparticle*charge_of_electron

phase_volume = Phase6DVolume(mass_of_macroparticle)
for filepath in args.phase_volume_files:
  filename = os.path.basename(filepath)
  if args.warp:
    phase_volume.injectFile(filepath,mass=mass_of_macroparticle,fieldnames=["x","y","z","px","py","pz","vx","vy","vz"])
  else:
...
binned_phase_volume.charge =charge_of_macroparticle
binned_phase_volume.printDebyeLengths()
