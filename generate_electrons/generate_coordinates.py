...
def generate_phase_volume(number_particles,polar_distribution,proximity_threshold,**kwargs):
  """
  Generates random particles according to the Three Step Model.
  """
  #proximity_grid = ProximityGrid(proximity_threshold)
  phase_volume = simulate_photoemission_processes(number_particles,**kwargs)
  #The phase volume only has momentum, no position.
...
def simulate_photoemission_processes(n,e_fermi=5.000E-6,e_work=4.45E-6,e_photon=4.65E-6,number_of_electrons_per_macroparticle=100,jennis_dist=False,**kwargs):
  """
...
if __name__ == "__main__":
  ...
  parser.add_argument('-p','--emission_profile', dest="profile", type=str, help='Describes the profile of the electrons in the radial direction.  Options are Gaussian (G), Elliptical (E), or Uniform (U).', default="Gaussian")
  ...
  #parser.add_argument('--pickle',dest="pickle",action="store_true",help="Return a pickle file of the dictionary.  The names will end in dict.pckl.", default=False)
  ...
  print "time sample obtained"
  phase_volume = generate_phase_volume(len(time_sample),polar_distribution,args.proximity_threshold,jennis_dist=args.jennis_dist,**photoemission_parameters)
  print "phase volume obtained"
  if len(time_sample) != len(phase_volume):
    raise Exception("The number of time points and phase points is not the same.")
  #if args.pickle:
  #array = []
  dict_array = []
  for i in range(len(time_sample)):
    ...
    #row = [time]
    dict = {"t":time}
    #row.extend(particle.asArray())
    dict.update(particle.asDict())
    #array.append(row)
    dict_array.append(dict)
  #pickle.dump(array,open(args.output_file+"_list.pckl",'w'))
  pickle.dump(dict_array,open(args.output_file+"_dict.pckl",'w'))
 # with open(args.output_file,'w') as f:
 #   for i in range(len(time_sample)):
 #     output = [str(time_sample[i])]
 #     output.append(str(phase_volume.particles[i]))
 #     f.write(" ".join(output) + "\n")
